...
def conciliacion_bd(request):
    ...
    url = settings.MANEJADOR_HC_URL + '/pacientes/' + str(id) + '/historia-clinica/'

    # mientras haya documentos por enviar
    while count != 0:
        # extrae los datos de la base de datos
        query = collection.find_one()
        json_data = query.get('data')
        req = requests.Request(query.get('method'), url, json=json_data)
        prepped = req.prepare()

        # TODO! Cuidado con ciclo infinito: podría estar mandando localmente
        if query.get('method') == 'POST':
            response = post_historia_clinica(prepped, json_data["id"], conciliacion=True)
        elif query.get('method') == 'PUT':
            response = put_historia_clinica(prepped, json_data["id"], conciliacion=True)
...
@csrf_exempt
def get_historia_clinica(request, id: int, conciliacion=False):
    """
    author: @c4ts0up
    ...
    try:
        # cambios locales no sincronizados y no en modo conciliación
...
@csrf_exempt
def post_historia_clinica(request, id: int, conciliacion=False):
    """
    author: @c4ts0up
    ...
    try:
        ...
        if response.status_code in [200, 201, 204]:
            response_data = response.text
            return JsonResponse({'user_id': 1, 'external_data': response_data, 'sent_to_cloud': True}, status=201)
...
@csrf_exempt
def put_historia_clinica(request, id: int, conciliacion=False):
    """
    author: @ppedreros
    ...
    try:
        ...
        if response.status_code in [200, 204]:
            response_data = response.text
            return JsonResponse({'user_id': 1, 'external_data': response_data, 'sent_to_cloud': True}, status=204)
...
@csrf_exempt
def clinical_history(request, id: int):
    if request.method == 'GET':
        return get_historia_clinica(request, id)
    elif request.method == 'POST':
        return post_historia_clinica(request, id)
    elif request.method == 'PUT':
        return put_historia_clinica(request, id)
    # error
    else:
        return JsonResponse({
            'user_id': 1,
            'external_data': 'Error: operación no reconocida',
            'sent_to_cloud': False
        }, status=500
        )
